...
int main(){
  ...
  //f(x) = e^x + x^3 -2
  double function1 = sub(add(Exp(userNumber),Pow(userNumber,3)),2);
  printf("The vulve of f(x) = e^x + x^3 -2  at the point %.4lf is:%.4lf\n" ,userNumber,function1);

  //f(x) = 3*x +2*x^2
  double function2 = add(mul(userNumber,3),Pow(mul(userNumber,2),2));
  printf("The vulve of f(x) = 3*x +2*x^2 at the point %.4lf is:%.4lf\n" ,userNumber,function2);

  //f(x) = (4*x^3)/5-2*x
  double function3 =  div(mul(Pow(userNumber,3),4),sub(mul(userNumber,2),5));
